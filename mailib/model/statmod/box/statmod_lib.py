...
#         return score
    ...
    def predict_model(self, predictors_loadings, predictors_scores,
                      model_loadings=None, model_scores=None, model_loading_curvfit=None, model_scores_curvfit=None,
                      observed_scores = False, dnn_model_scores =None, rnn_model_scores =None, use_predict_func_loading = None, nb_PC = None, qm_maps=None):
...
#              loading_est = fit_loading(predictor_loadings, *p)
        ...
        for PC_nb in range(1,nb_PC+1):
            ...
            # get the predictors
            x_scores = predictors_scores.loc[:, model_scores['predictor'][PC_nb]]
        ...
        for column in df_PCs:
            ...
            if not manual_selection:
                for i in range(lim_nb_predictors):
                    ...
                    if log:
                        dic_log[column][str(i)] = scores_with_candidates
    ...
    def skill_model(self, df_verif, res, metrics, params_loadings=None, params_scores=None,
                     plot_bias=None, hours=False, plot_summary=False, summary=None, mean_data=False, type='predicted'):
        """
        DESCRIPTION
            Compute bias and RMSE to assess the model performance 
        INPUT
        ...
        if  isinstance(mean_data, pd.core.series.Series):
            data = data + mean_data[:,None]
        
        df_rec = pd.DataFrame(data, columns = df_verif.columns, index =res['index']) # should improve this
...
#                 sns.heatmap(score)
    ...
    def dnn_model(self, X, Y):
        ...
        for pc_nb in Y.columns:
            ...
            print('='*100)
            model, history = DNN_ribeirao(X, Y_scaled)
    ...
    def rnn_model(self, X, Y, SEQUENCE_LENGTH=8):
        models =[]
        predictor_names = []
        # Scaling
        scaler_x = MinMaxScaler(feature_range=(0, 1))
        ...
        for pc_nb in Y.columns:
            ...
            model, history = RNN_ribeirao(X_lag, Y_scaled)
